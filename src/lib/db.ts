...
function init(db: Database.Database) {
  ...
  const imagePromptRow = db.prepare("SELECT value FROM settings WHERE key = 'gemini_image_prompt'").get();
  if (!imagePromptRow) {
    db.prepare("INSERT INTO settings (key, value) VALUES ('gemini_image_prompt', ?)").run(
